"""Command-line entry point: ``scabopdf-epub-extract``.

Runs the Layer 1 EPUB IPZS backend on a Normattiva EPUB export and
writes the corresponding JSON document conforming to the ScaboPDF
v0.7.0 schema. The CLI orchestrates the three phases — detect + parse,
emit, optional double validation, write — directly so it can
...
CLI (``scabopdf-extract``) and the XML AKN CLI (``scabopdf-xml-extract``)
convention: progress messages and unexpected-error tracebacks go to
**stderr**; the final structured summary (one ``key: value`` per line,
VoiceOver-friendly and shell-parseable) goes to **stdout**.

Exit codes
----------
0
    Success. Includes the ``OK_FLAT_ATTACHMENT`` verdict — the parser
    produces a valid ``Document`` for FLAT_ATTACHMENT inputs and
    signals the lost comma structure via a closed warning, mirroring
    the XML AKN CLI's treatment of ``FRAGMENTED``.
1
    :class:`EpubIpzsParseError` (``NOT_IPZS_EPUB`` or
    ``INVALID_EPUB``), :class:`EmissionError`, or any other unexpected
    exception.
2
...
PROGRESS_PREFIX = "[scabopdf-epub-extract]"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    ----------
    argv
        Argument vector. ``None`` (the default) means use
        ``sys.argv[1:]``, matching how argparse is normally invoked.
    ...
    output_path = Path(args.output) if args.output else epub_path.with_suffix(".json")
    ...
    try:
        document = _run_pipeline(epub_path, output_path, validate=validate, timer=timer)
    except EpubIpzsParseError as exc:
...
def format_summary(
    ...
    """Format the stdout summary of an emitted EPUB IPZS document.

    Returns a multi-line ``key: value`` string, one entry per line,
    terminated by a trailing newline. The format is intentionally
    minimal: no colors, no ASCII tables, no padding — a blind user
    with VoiceOver can scan it row by row and a shell script can grep
    or awk it without trouble.

    Keys, in order: ``document_id``, ``profile_id``, ``schema_version``,
    ``n_nodes_total``, ``n_warnings``, ``output_path``. The PDF CLI's
    ``pages_pdf`` key is intentionally omitted because the EPUB backend
    stubs it at zero (EPUB has no physical-page concept) — surfacing it
    would mislead the user. Symmetric to the XML AKN CLI.
...
def _run_pipeline(
    ...
    *,
    ...
    """Run the EPUB IPZS backend with progress reporting.
    ...
    * :class:`EpubIpzsParseError` propagates unchanged so the caller
      can surface the detector's prose explanation directly.
    * Any other I/O, validation, or runtime failure is wrapped as
    ...
    try:
        ...
        timer.next("emitting")
        scabopdf_document = to_scabopdf_document(result, epub_path)
        ...
        raise EmissionError(
            f"emission failed for {str(epub_path)!r} -> {str(output_path)!r}: {exc}"
        ) from exc
...
class _ProgressTimer:
    ...
    prints ``"<phase>... [<elapsed>s]"`` where ``<elapsed>`` is the
    time spent on the *previous* phase. ``finish`` prints ``"done
    [<total>s total]"``. When ``verbose`` is False every method is a
    silent timer update.
    """
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="scabopdf-epub-extract",
        description=(
            "ScaboPDF Layer 1 EPUB IPZS backend — parse a Normattiva "
            "EPUB export into a JSON document conforming to schema "
            f"v{SCHEMA_VERSION}."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Path to the output JSON file. Defaults to the EPUB's path "
