...
## 10. Addendum cross-fixture v2.31 (25 maggio 2026)

L'analisi iniziale di luglio 2026 era ristretta a tre fixture esplorative (codice_penale, legge_capitali, legge_finanziaria_2007). La diagnostica Fase 0 della sessione di landing del backend EPUB IPZS (25 maggio 2026 sera, debt xv chiuso) ha esteso lo scan a tutte le 11 fixture cross-corpus (8 in `normattiva_calibration/` + 3 in `normattiva_exploration/`) tramite lo script `scripts/diagnose_epub_families.py`. Le conclusioni cross-atto del documento originale sono **confermate al 100% cross-corpus**: ogni fixture è EPUB 2.0, mimetype `application/epub+zip` esatto, generator `EPUBLib version 3.0`, dc:creator `Istituto Poligrafico e della Zecca dello Stato`, zero cross-reference interni `<a href>` (i 12 external link della finanziaria 2007 sono link a PDF Normattiva via `attachment-url-link`, gli unici, replicati anche su legge_bilancio_2023 con 66 occorrenze).

Le **due famiglie strutturali** identificate sulle 3 fixture iniziali (STRUCTURED vs FLAT_ATTACHMENT) si confermano partizionare il corpus in modo netto: 9 fixture STRUCTURED (legge_56_2007 con 2 articoli, gelli_bianco 18, dlgs_231 109, bilancio_2023 29, codice_strada 266, codice_procedura_penale 906, tuf 563, legge_capitali 28, legge_finanziaria_2007 con 14 article-num split-pages del singolo art. 1 + 1297 commi) e 2 fixture FLAT_ATTACHMENT (codice_penale 987 attachment-just-text + 3 transitional article-num, codice_civile 3256 attachment-just-text + 2 promulgazione). La terza famiglia "SPLIT_ARTICLE" inizialmente ipotizzata per la finanziaria 2007 si rivela non strutturalmente distinta: la finanziaria si lascia trattare come STRUCTURED con un warning di "duplicate article id" se rilevato — il dispatcher del detector è semplificato a 4 verdetti (`OK_STRUCTURED`, `OK_FLAT_ATTACHMENT`, `NOT_IPZS_EPUB`, `INVALID_EPUB`) e non a 5.

Soglie del detector calibrate sull'inventario cross-fixture: `ATTACHMENT_FLAT_MIN = 50` (caso FLAT più piccolo: CP 987 → 20x cushion; caso STRUCTURED max stray: bilancio_2023 10 → 5x cushion); `ARTICLE_NUM_FLAT_MAX = 5` (CP=3, CC=2; minimo STRUCTURED: legge_56_2007 con 2 — la coesistenza è risolta dalla AND con la prima soglia, perché legge_56_2007 ha zero attachment-just-text); `ARTICLE_NUM_STRUCTURED_MIN = 1`. Zero falsi positivi, zero falsi negativi sull'intera 11-fixture corpus.

Altri pattern cross-atto rilevanti per il parser: `signature-*-akn` presente su 7 fixture su 11 (assente su cp/cc/cpp che sono codici regio-decretali pre-Costituzione, e completamente assente sui codici antichi degradati a `attachment-just-text`); `art_abrogato-akn` presente su 5 fixture (cpp=15, cc=173, cp=91, codice_strada=12, tuf=64; assente su tutte le leggi recenti — pattern empirico "art_abrogato_count > 0 solo su codici di età decennale+"); `art_aggiornamento-akn` presente su quasi tutte le fixture eccetto le sole legge_56_2007 e legge_capitali (atti recenti senza modifiche post-pubblicazione). Vedi `docs/EPUB_PARSING.md` per il dettaglio del parser e delle 11 baseline byte-for-byte E-001..E-011.
