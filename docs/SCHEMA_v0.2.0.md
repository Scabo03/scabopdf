...
La versione 0.2.0 è **dichiaratamente instabile**. Riflette quello che la pipeline produce oggi dopo i §§ 1–6, § 8 e § 9 di `ARCHITECTURE.md`, più il primo step generico del § 7 (`dehyphenate_with_log`). Rispetto alla 0.1.0 il delta è additivo: un singolo nuovo campo top-level `transformations` che porta il log reversibile delle trasformazioni testuali applicate in post-processing. Tutto ciò che gli undici step profile-specific del § 7 produrranno quando i plugin di corpus arriveranno, e l'ulteriore arricchimento di metadati ed apparati, non è ancora qui.
...
**Limite di applicabilità di `dehyphenate_with_log`.** Lo step generico introdotto in 0.2.0 lavora sulla stringa piatta `Node.text` e cerca un pattern `letter+ [-­] \n letter+`. È quindi capace di intercettare le hyphenation di fine riga **solo se** `Node.text` contiene letteralmente il carattere `\n` fra le righe. Questo è il caso tipico per output OCR (gli OCR scrivono le righe separate da `\n`, e infatti la tabella di `ARCHITECTURE.md § 7.1` lo accosta solo al profilo `enciclopedia_storica`). Sui PDF tipograficamente impaginati come Patriarca-Benazzo o Mosconi-Campiglio, PyMuPDF rappresenta le interruzioni di riga **geometricamente**: ciascuna riga è uno `Span` con `y0` distinto e `Span.text` non contiene mai `\n`. Il `_block_text` di tier 1 concatena gli span senza separatore, quindi `Node.text` resta privo di `\n` e lo step non trova match. Conseguenza pratica: su quei fixture il blocco `transformations` resta vuoto anche quando il profilo dichiara `dehyphenate_with_log`. La funzionalità non è bacata, è semplicemente fuori scopo per dati non-OCR — quando un futuro profilo OCR la attiverà, il `transformations` log si popolerà come previsto. Il test `test_pipeline_with_dehyphenation_on_patriarca_is_a_noop` in `pipeline/tests/integration/test_layer1_end_to_end.py` congela questa proprietà come regression test.
...
<!-- Esempio estratto da Patriarca-Benazzo via scabopdf-extract il 2026-05-12. Selezione di 6 nodi non consecutivi (indici 0, 1, 2, 19, 20, 21 nella foresta originale di 1616 nodi). Aggiornato a 0.2.0 il 2026-05-13: aggiunta della voce "transformations": [] che riflette il blocco vuoto emesso quando il profilo (unknown_generic) non dichiara step di post-processing. -->

Quando il primo plugin di tier 2 sarà operativo e dichiarerà almeno uno step di post-processing, l'esempio della sezione 4 verrà ripreso con una `transformations` non vuota e un sub-tree gerarchico realistico — `HEADING_1` con figli `HEADING_2`, `BODY`, `CROSS_REFERENCE`/`NOTE` collegati da `apparatus_refs` — accanto all'esempio piatto qui sopra, così la disciplina della versione resterà visibile in entrambe le forme.

## 5. Cosa NON è in v0.2.0

Lo schema v0.2.0 è ancora volutamente parziale. I prossimi bump aggiungeranno, in ordine non garantito:
