...
`manuale_giappichelli` usa il prefisso `plugin:giappichelli:` e sei voci:
...
- `plugin:giappichelli:body_note_block_glued_block_<idx>_page_<p>` — un blocco emesso da PyMuPDF combina spans di body e spans di nota nello stesso block, con la firma tipografica delle note (size ridotta) intercalata alla firma del body. Il plugin diagnostica l'occorrenza ma non splitta il blocco — lo split richiede re-segmentazione delle span e va valutato in una sessione dedicata. Sulla fixture Mandrioli-Carratta Vol. III il warning fired per ~2000 blocchi, ben oltre il < 1 % stimato dall'analisi: la frequenza dell'evento sulla fixture reale rende imprescindibile l'eventuale implementazione di uno step di split, oggi rinviata.
