...
`manuale_giappichelli` copre dopo la sessione di consolidamento del 2026-05-17 l'intera serie Mandrioli-Carratta in quattro volumi, accettando entrambe le pipeline produttive che Giappichelli ha alternato nel tempo: i volumi I e II nella ventesima edizione, prodotti con il flusso Photoshop più datato che rende le note a 7.98pt, e i volumi III e IV nella trentesima edizione, prodotti con il flusso Adobe InDesign 20.x che le rende a 9.0pt. La doppia famiglia tipografica è codificata nel plugin con la coppia `NOTE_BODY_SIZES = (7.98, 9.0)`, che ammette entrambi i regimi senza dover ramificare il codice. Il meccanismo nuovo introdotto da questa sessione è il body+note glued splitter in `refine_reconstruction`, che ricostruisce gli apparati incollati ai blocchi di body dalla segmentazione di PyMuPDF; un test di integrazione di regressione protezione, condotto sulla fixture Marotta "Cittadinanza romana" — un volume pubblicato da Giappichelli ma estraneo alla collana Mandrioli — garantisce che il plugin non promuova un corpus che non gli appartiene. Il prefisso è `plugin:giappichelli:` e il vocabolario chiuso conta sette voci:

- `plugin:giappichelli:outline_paragraph_mismatch_node_<id>` — l'outline embedded del PDF dichiara un titolo di paragrafo che non matcha alcun blocco classificato come `HEADING_3`/`HEADING_4` nel tree dopo `refine_reconstruction`. Warning informativo per audit: l'outline tagged Giappichelli a quattro livelli (113 voci sul Vol. III) non è esaustivo e può divergere dalla tipografia effettiva del corpo.
- `plugin:giappichelli:chapter_summary_unparseable_node_<id>` — un `CHAPTER_SUMMARY` è stato riconosciuto per signature (small-caps + size Mandrioli) ma il suo testo non ha potuto essere decomposto in voci `(number, title)` con il regex del plugin. Il nodo viene emesso con `items: null` e questo warning aggiunto.
- `plugin:giappichelli:chapter_title_not_adjacent_block_<idx>_page_<p>` — il blocco dell'intestazione CAPITOLO atteso immediatamente dopo l'etichetta SOMMARIO non è risultato adiacente come sibling nel tree dopo la ricomposizione. Il nodo viene emesso comunque, senza fusione del titolo, e il warning segnala la rottura del pattern editoriale standard.
- `plugin:giappichelli:inline_cross_reference_minted_node_<id>_page_<p>` — il plugin ha mintato un Node `CROSS_REFERENCE` sintetico (id nel pattern `node_NNNN` mintato a partire dal massimo contatore tier 1) per un marker testuale parentesizzato `(N)` trovato dentro un BODY block via regex `\((\d+)\)`. Warning informativo per audit della catena di re-parsing → resolver tier 1 binding. La differenza chiave rispetto al warning analogo del Mosconi è che il marker non è il flag SUPERSCRIPT delle span ma il pattern testuale del Node.text.
- `plugin:giappichelli:marginal_gloss_outside_margin_block_<idx>_page_<p>` — un blocco con signature `AGaramondPro-BoldItalic` 8.5pt (la firma canonica delle glosse marginali Giappichelli) è stato osservato con coordinate `x` fuori dal range del margine sinistro atteso. Il blocco è lasciato `UNCLASSIFIED` e il warning segnala la divergenza dalla convenzione tipografica del corpus.
- `plugin:giappichelli:body_note_block_glued_block_<idx>_page_<p>` — un blocco emesso da PyMuPDF combina spans di body e spans di nota nello stesso block, con la firma tipografica delle note (size ridotta) intercalata alla firma del body. Il warning viene emesso in classificazione, nel momento in cui il pattern incollato è riconosciuto per la prima volta, e documenta il problema strutturale a monte indipendentemente dall'esito del recupero a valle. Sulla fixture Mandrioli-Carratta Vol. III il warning fired per ~2000 blocchi, ben oltre il < 1 % stimato dall'analisi.
- `plugin:giappichelli:body_note_split_minted_node_<id>_page_<p>` — il body+note glued splitter implementato in `refine_reconstruction` (introdotto nel commit `c01661e` del 2026-05-17) ha mintato un Node `NOTE` sintetico per recuperare il contenuto di apparato annegato in un blocco incollato. Il warning viene emesso una volta per ogni Node sintetico, dove il marker che ha guidato lo split è il pattern `(N)` trovato nelle span dimensionate come nota nel blocco di origine. La coesistenza con `body_note_block_glued_block_<idx>_page_<p>` è deliberata: il primo warning fotografa il problema strutturale a monte (PyMuPDF ha fuso due blocchi in uno), il secondo fotografa il recupero a valle (il plugin ha materializzato il contenuto annegato come Node indirizzabile). Il delta tra i due conteggi sulla stessa fixture è un termometro diagnostico dei "glued block che lo splitter non è riuscito a recuperare", tipicamente perché il marker `(N)` manca sul frammento di continuazione o perché il blocco appartiene a una fusione di nota a cavallo di pagina che lo splitter per-blocco aggira per costruzione. I numeri empirici sul Vol. III, ~2000 glued contro 466 split-minted, lasciano il gap in carico al futuro promozionamento di `merge_cross_page_notes` da placeholder a step reale, dove la fusione cross-page potrà recuperare la maggior parte dei casi residui.
