...
> Versione: 1.4 (11 maggio 2026, dopo chiusura del § 6 apparatus resolution tier 1 generico + dispatch al tier 2 e seconda fixture reale operativa Mosconi-Campiglio)
...
Il progetto è entrato in fase di sviluppo del Layer 1 (pipeline Python) il 9 maggio 2026 con lo scaffold del repository monorepo. All'11 maggio 2026 sono completi e su `origin/main` i §§ 1-6 di Layer 1: project setup (`d82e8b6`), profiling foundations (`96cdc8e`), block extraction (`07b0d07` + `d9aa37b` + `9e1d360`), block classification tier 1 + dispatch al tier 2 (`040fdfd` + `e8ead6a`), structural reconstruction tier 1 + dispatch al tier 2 (`ab9b9e4` + `34c2363`), apparatus resolution tier 1 + dispatch al tier 2 (`62688f4` + `27f7ceb`); a queste si aggiungono l'infrastruttura per le fixture private (`e17e823`), l'housekeeping di formattazione su `test_pymupdf_adapter.py` (`b39f039`) e il primo integration test end-to-end Layer 1 con la fixture Patriarca-Benazzo (`e468fe2`), poi esteso a due fixture (Patriarca + Mosconi-Campiglio) nella sessione di § 6. Il Layer 2 (React Native in `app/`) e la CI/pre-commit non sono ancora inizializzati: rimandati a sessione dedicata in cui sarà disponibile MacInCloud per il primo build effettivo iOS.
...
**Codice di Layer 1**: `pipeline/` è uno scheletro funzionante con sei moduli implementati (`extraction/`, `profiling/`, `classification/`, `reconstruction/`, `apparatus/`, `schema/`) e tre stub vuoti per i moduli a valle (`emission/`, `postprocessing/`, `utils/`), più `profiles/` con il solo `unknown_generic` aggiornato per implementare `refine_classification`, `refine_reconstruction` e `refine_apparatus` come pass-through. 126 test verdi totali (124 unit + 2 integration), coverage 95% complessiva. Il marker PEP 561 `py.typed` è in `src/scabopdf_pipeline/`, mypy strict è pulito sia su `src/` che su `tests/`. La cartella `pipeline/tests/fixtures/private/` (gitignored) ospita due fixture reali: `patriarca_benazzo.pdf` (23 MB, 504 pp., apparato zero) e `mosconi_campiglio.pdf` (6.6 MB, 613 pp., apparato denso 965 note + 593 marginali + 420 box). Il file `pipeline/tests/fixtures/README.md` (versionato) documenta la convenzione private/public e lo skip pulito dei test integration quando la fixture non è disponibile localmente.
...
### Sessione che ha prodotto questa versione del carryover (11 maggio 2026, dodicesimo aggiornamento — versione 1.4)
...
- **Layer 1 §§ 1-6 chiusi e su `origin/main`** all'11 maggio 2026, due fixture reali operative (`patriarca_benazzo.pdf` zero apparato + `mosconi_campiglio.pdf` apparato denso) e due integration test end-to-end Layer 1 verdi. Quality gate del momento: **126 test verdi totali (124 unit + 2 integration)**, coverage 95% complessiva, ruff check e ruff format check puliti, mypy strict pulito su `src/` e `tests/`. **Prossimo task naturale: § 8 JSON schema definitivo + § 9 emission CLI**, da fare come due sessioni separate per pulizia (chiudere il contratto Layer 1 ↔ Layer 2 prima di proliferare i plugin di corpus). Alternative residue: primo plugin di corpus (`manuale_zanichelli_giuridica` per Patriarca, profilo più semplice del corpus, fixture già disponibile, primo esercizio reale del meccanismo plugin-based tier 2 oggi solo pass-through); terza fixture reale per regression (Mandrioli-Carratta Vol. IV come secondo campione di stress test, utile anche per coprire il branch Custom-encoding di `pymupdf_adapter.py`); Blocco B (init React Native in `app/`) se si prevede sessione MacInCloud nel breve.
- **Regole `CLAUDE.md` sono vincolanti**: niente menu interattivi, niente decisioni autonome su design/library/schema/architettura, chiarificazioni in prosa lineare con trade-off espliciti. Le sessioni di § 3, § 4, § 5, fixture-reali e § 6 hanno applicato il pattern (sette questioni in prosa per § 3, sei per § 4, otto per § 5, tre + una nota integrativa per fixture-reali, quattro + due scoperte emerse a metà implementazione per § 6, risposte ricevute, poi implementazione end-to-end). È il modello per le sessioni successive. **Pattern raccomandato per scoperte a metà implementazione**: fermarsi e segnalare in prosa appena emerge l'incongruenza tra piano e realtà (codice esistente, dati empirici della fixture), non procedere autonomamente — è coerente con la nota CLAUDE.md "Se durante l'estensione dell'integration test scopri bug o edge case nel codice di produzione precedente, FERMATI e segnalalo in prosa".
