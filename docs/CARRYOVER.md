...
> Versione: 1.2 (11 maggio 2026, dopo chiusura del Layer 1 § 5 — pipeline Python operativa fino alla structural reconstruction tier 1 con dispatch al tier 2)
...
**Codice di Layer 1**: `pipeline/` è uno scheletro funzionante con cinque moduli implementati (`extraction/`, `profiling/`, `classification/`, `reconstruction/`, `schema/`) e quattro stub vuoti per i moduli a valle (`apparatus/`, `emission/`, `postprocessing/`, `utils/`), più `profiles/` con il solo `unknown_generic` aggiornato per implementare `refine_classification` e `refine_reconstruction` come pass-through. 102 test unit verdi, coverage 97% complessiva. Il marker PEP 561 `py.typed` è in `src/scabopdf_pipeline/`, mypy strict è pulito sia su `src/` che su `tests/`.
...
### Sessione che ha prodotto questa versione del carryover (11 maggio 2026, decimo aggiornamento — versione 1.2)
...
- **Layer 1 §§ 1-5 chiusi e su `origin/main`** all'11 maggio 2026. Quality gate del momento: 102 test verdi, coverage 97% complessiva, ruff e mypy strict puliti su `src/` e `tests/`. **Prossimo task naturale: § 6 apparatus resolution** (note cross-reference resolver, cross-page note merging, marginal heading positioning, marginal ellipsis recomposition, box association al body, marginal gloss association — vedi ARCHITECTURE.md § 6). Alternative: § 8 JSON schema + § 9 emission (chiudere prima il contratto e il writer prima di § 6/7 può semplificare il debug del round-trip), Blocco B (init React Native in `app/`) se si prevede una sessione MacInCloud nel breve, oppure sessione fixture reali (rinviata da molte sessioni, sempre più rilevante per coprire il branch Custom-encoding di § 3 e l'integration test round-trip char-count del checklist § 3).
- **Regole `CLAUDE.md` sono vincolanti**: niente menu interattivi, niente decisioni autonome su design/library/schema/architettura, chiarificazioni in prosa lineare con trade-off espliciti. Le sessioni di § 3, § 4 e § 5 hanno applicato il pattern (sette questioni in prosa per § 3, sei per § 4, otto per § 5, risposte ricevute, poi implementazione end-to-end). È il modello per le sessioni successive.
