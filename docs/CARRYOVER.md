...
> Versione: 1.5 (12 maggio 2026, dopo chiusura del § 8 JSON schema contract v0.1.0 — modelli Pydantic v2 autoritativi, generatore di `shared/schema.json`, validatore a due livelli, documentazione narrativa e policy di versioning SemVer)
...
Il progetto è entrato in fase di sviluppo del Layer 1 (pipeline Python) il 9 maggio 2026 con lo scaffold del repository monorepo. Al 12 maggio 2026 sono completi e su `main` locale (in attesa di push su `origin/main`) i §§ 1-6 di Layer 1 più il § 8 JSON schema contract: project setup (`d82e8b6`), profiling foundations (`96cdc8e`), block extraction (`07b0d07` + `d9aa37b` + `9e1d360`), block classification tier 1 + dispatch al tier 2 (`040fdfd` + `e8ead6a`), structural reconstruction tier 1 + dispatch al tier 2 (`ab9b9e4` + `34c2363`), apparatus resolution tier 1 + dispatch al tier 2 (`62688f4` + `27f7ceb`), e il § 8 JSON schema contract v0.1.0 in tre commit (`86c19c1` produzione + `03c66bc` test + `a697704` documentazione); a queste si aggiungono l'infrastruttura per le fixture private (`e17e823`), l'housekeeping di formattazione su `test_pymupdf_adapter.py` (`b39f039`) e il primo integration test end-to-end Layer 1 con la fixture Patriarca-Benazzo (`e468fe2`), poi esteso a due fixture (Patriarca + Mosconi-Campiglio) nella sessione di § 6. Il Layer 2 (React Native in `app/`) e la CI/pre-commit non sono ancora inizializzati: rimandati a sessione dedicata in cui sarà disponibile MacInCloud per il primo build effettivo iOS.
...
**Codice di Layer 1**: `pipeline/` è uno scheletro funzionante con sei moduli implementati (`extraction/`, `profiling/`, `classification/`, `reconstruction/`, `apparatus/`, `schema/` ora completo con `contract.py` e `validator.py`) e due stub vuoti per i moduli a valle (`emission/`, `postprocessing/`), più `profiles/` con il solo `unknown_generic` aggiornato per implementare `refine_classification`, `refine_reconstruction` e `refine_apparatus` come pass-through. Aggiunto anche `pipeline/scripts/generate_schema.py` come utility fuori da `src/` per rigenerare `shared/schema.json` dai modelli Pydantic. 153 test verdi totali (151 unit + 2 integration), coverage 96% complessiva. Il marker PEP 561 `py.typed` è in `src/scabopdf_pipeline/`, mypy strict è pulito sia su `src/` che su `tests/`. La cartella `pipeline/tests/fixtures/private/` (gitignored) ospita due fixture reali: `patriarca_benazzo.pdf` (23 MB, 504 pp., apparato zero) e `mosconi_campiglio.pdf` (6.6 MB, 613 pp., apparato denso 965 note + 593 marginali + 420 box). Il file `pipeline/tests/fixtures/README.md` (versionato) documenta la convenzione private/public e lo skip pulito dei test integration quando la fixture non è disponibile localmente.

`SCHEMA_v0.1.0.md` — **PRODOTTO** (commit `a697704`, 12 maggio 2026). Riferimento narrativo dello schema JSON v0.1.0: scopo del documento e relazione con `shared/schema.json`, stato pre-1.0, descrizione campo-per-campo, esempio JSON realistico coerente con Patriarca e Mosconi, lista di cosa NON è in v0.1.0 (span granulari, transformations, metadati editoriali ricchi, detection signals, regime acustico A/B/C/D, page_book BIC, strutture profile-specific, `associated_body_id` e `gloss_id`), e sezione 6 di disciplina di lavoro (650 parole, vincolo non negoziabile sull'allineamento codice ↔ contratto).

`json-schema-versioning.md` — **PRODOTTO** (commit `a697704`, 12 maggio 2026). Policy SemVer applicata allo schema (patch/minor/major), stato corrente 0.1.0 instabile, roadmap verso 1.0.0 al completamento funzionale del Layer 1 (tutti i plugin di corpus + § 7 post-processing), procedura in sette tappe per ogni modifica allo schema, riferimento al drift test come prima linea di difesa e a `validate_against_schema` come seconda.
...
- `SCHEMA_CHANGELOG.md` — alla prima modifica reale dello schema dopo il bootstrap v0.1.0
...
### Sessione che ha prodotto questa versione del carryover (12 maggio 2026, tredicesimo aggiornamento — versione 1.5)
...
- **Layer 1 §§ 1-6 chiusi su `origin/main` + § 8 JSON schema contract v0.1.0 chiuso su `main` locale** al 12 maggio 2026, due fixture reali operative (`patriarca_benazzo.pdf` zero apparato + `mosconi_campiglio.pdf` apparato denso) e due integration test end-to-end Layer 1 verdi (invariati). Quality gate del momento: **153 test verdi totali (151 unit + 2 integration)**, coverage 96% complessiva, ruff check e ruff format check puliti, mypy strict pulito su `src/` e `tests/`. **Prossimo task naturale: § 9 emission CLI** — il convertitore puro `Document Python → ScabopdfDocument Pydantic`, il serializzatore JSON e l'entry point `scabopdf-extract` come prima integrazione end-to-end Layer 1 (extraction → profiling → classification → reconstruction → apparatus → emission), con test integration sulle due fixture reali che produrranno JSON validi contro `shared/schema.json`. Alternative residue: primo plugin di corpus (`manuale_zanichelli_giuridica` per Patriarca, profilo più semplice del corpus, fixture già disponibile, primo esercizio reale del meccanismo plugin-based tier 2 oggi solo pass-through); terza fixture reale per regression (Mandrioli-Carratta Vol. IV come secondo campione di stress test, utile anche per coprire il branch Custom-encoding di `pymupdf_adapter.py`); Blocco B (init React Native in `app/`) se si prevede sessione MacInCloud nel breve.
- **Regole `CLAUDE.md` sono vincolanti**: niente menu interattivi, niente decisioni autonome su design/library/schema/architettura, chiarificazioni in prosa lineare con trade-off espliciti. Le sessioni di § 3, § 4, § 5, fixture-reali, § 6 e § 8 hanno applicato il pattern (sette questioni in prosa per § 3, sei per § 4, otto per § 5, tre + una nota integrativa per fixture-reali, quattro + due scoperte emerse a metà implementazione per § 6, nove questioni + due deviazioni dichiarate per § 8, risposte ricevute, poi implementazione end-to-end). È il modello per le sessioni successive. **Pattern raccomandato per scoperte a metà implementazione**: fermarsi e segnalare in prosa appena emerge l'incongruenza tra piano e realtà (codice esistente, dati empirici della fixture), non procedere autonomamente — è coerente con la nota CLAUDE.md "Se durante l'estensione dell'integration test scopri bug o edge case nel codice di produzione precedente, FERMATI e segnalalo in prosa".
- **Disciplina dello schema JSON è non negoziabile** (cristallizzata nella sezione "Schema discipline" di `CLAUDE.md` e nella sezione 6 di `docs/SCHEMA_v0.1.0.md`): lo schema 0.1.0 in `shared/schema.json` è il contratto vincolante fra Layer 1 e Layer 2. Ogni modifica al codice di produzione capace di cambiare la struttura dell'output (`extraction/types.py`, `classification/types.py`, `reconstruction/types.py`, `apparatus/types.py`, `profiling/profile.py`, `schema/categories.py`) richiede verifica esplicita di conformità con `pipeline/src/scabopdf_pipeline/schema/contract.py` **nella stessa sessione**. Pattern operativo: modifica codice → aggiorna `contract.py` se serve → rigenera `shared/schema.json` con `python pipeline/scripts/generate_schema.py` → aggiorna `SCHEMA_v0.1.0.md` (e `SCHEMA_CHANGELOG.md` quando esisterà) → verifica drift test in `tests/unit/schema/test_generate_schema.py`. Il drift test fallisce se i due divergono, rendendo impossibile la violazione silenziosa.
- **Convenzioni di codice consolidate dai §§ 2, 3, 4, 5 e 6 (da rispettare a valle)**: dataclass `frozen` per i contenitori interni (`kw_only=True` quando l'ordine semantico dei campi richiede un default-prima-di-non-default — caso `ClassifiedBlock`, `Document`, `Node`, `ApparatusRef`), albero rigoroso `children` come tuple per immutabilità profonda (caso `Node`), Pydantic `BaseModel` con `model_config = ConfigDict(frozen=True)` per ciò che esce in JSON; un solo modulo per ogni I/O esterno (`pymupdf_adapter.py` è l'unico import di `fitz`); `__init__.py` di modulo re-esporta solo i simboli pubblici; bool derivati da bitfield esposti come `@property` sopra il campo crudo; ranges half-open `(start, end)` per linkare contenitori a liste flat e indici flat per fare riferimento a un elemento (es. `ClassifiedBlock.block_index` punta a `ExtractionResult.blocks`, non al `Block.block_index` per-pagina; `Node.block_indices` tuple di indici flat per legare il nodo ai blocchi originali; `ApparatusRef.target_node_id` come stringa, non come riferimento tipato a Node, per mantenere `apparatus.types` foglia tipologica); filtro precoce dei dati irrilevanti già in extraction quando ARCHITECTURE.md è esplicita su "Otherwise discarded"; vocabolari chiusi documentati come costanti modulo-livello (es. `TIER1_REASONS`, `TIER1_WARNING_TEMPLATES` in reconstruction e in apparatus) quando il dominio dei valori è enumerabile e citato in ARCHITECTURE.md; pattern due-passi per costruire alberi frozen (classe interna `_NodeBuilder` mutabile durante l'assemblaggio o la modifica, `to_frozen()` ricorsivo alla fine — applicato sia in reconstruction sia in apparatus, con `_thaw` ricorsivo come ingresso quando l'apparatus parte da un Document già frozen); id deterministici e leggibili sull'ordine di emissione (`"node_NNNN"` zero-padded); le modifiche di interfaccia astratta (es. nuovo metodo `@abstractmethod`) si bundlano con l'aggiornamento dei test che le rendono compilabili, per non lasciare lo stato intermedio rotto tra commit produzione e commit test; gli import puramente tipo si spostano sotto `TYPE_CHECKING` quando servono a rompere cicli (caso `Document` in `profiling/plugin.py` dopo che `reconstruction/__init__.py` re-esporta `reconstruct`; caso `ApparatusRef` in `reconstruction/types.py` dopo che `apparatus/__init__.py` esegue il `resolver` che importa Node).
