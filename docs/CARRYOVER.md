...
> Versione: 0.9 (maggio 2026, dopo redazione di `ARCHITECTURE.md` e `ANALYSIS_MANUALI_OVERVIEW.md`)
...
Il progetto è in fase di design e analisi pre-sviluppo. Non è stato scritto ancora nessun codice.
...
`ARCHITECTURE.md` — **PRODOTTO** in versione 0.1 (maggio 2026). Copre Layer 1 (Python pipeline) + Layer 2 (React Native app) come guida operativa con checklist organizzate per fase di sviluppo, in inglese, livello di dettaglio medio. 13 sezioni operative + principi architetturali iniziali.
...
### Sessione che ha prodotto questa versione del carryover (maggio 2026, settimo aggiornamento — versione 0.9)
...
- I PDF DeJure vengono trattati dal contatore della chat come immagini, quindi occupano quota molto velocemente — pianificare le sessioni di conseguenza, caricando un solo PDF per turno quando possibile e chiudendo la chat quando il contatore si esaurisce
...
- **Claude Code è il prossimo strumento da coinvolgere**: la fase di design e analisi pre-sviluppo è conclusa. Tutto il corpus è coperto, le decisioni architetturali sono cristallizzate in `ARCHITECTURE.md`, lo schema JSON è dichiarato come contratto Layer 1 ↔ Layer 2. Il prossimo passo operativo è il setup del repository monorepo (struttura in `ARCHITECTURE.md` § 1.1) e l'inizio dell'implementazione del Layer 1 (pipeline Python).
- **Richiesta esplicita dell'utente: messaggi e commenti sintetici e concisi** (l'utente non sempre comprende terminologia tecnica). Aggiornamenti diretti sui file di analisi piuttosto che lunghi riepiloghi nei messaggi di chat.
- **PyMuPDF è ora lo strumento standard di analisi in chat**. Tutti i numeri di `ANALYSIS_GIUFFRE_ENCICLOPEDIA.md`, dei sei file `ANALYSIS_*` dei manuali, e di `ANALYSIS_MANUALI_OVERVIEW.md` sono misurati direttamente dal filesystem. I numeri di altri file di analisi (Massime, Note a Sentenza, Dottrina, Codici) restano stimati visivamente o per inferenza dal pipeline — possono essere consolidati in sessioni future caricando i PDF originali e rieseguendo PyMuPDF.
- Le note a sentenza sono state chiuse senza terzo campione perché l'utente ha valutato il rischio sufficientemente basso. Se in futuro emergessero edge case nuovi (heading non numerati, liste autori multiple), riaprire `ANALYSIS_DEJURE_NOTE.md` per integrazione mirata.
- **Voci EdD chiuse dopo quarta ondata maggio 2026**: il profilo è solido, copre dieci campioni in 60 anni di evoluzione editoriale (1962-2025). Eventuali edge case nuovi possono essere integrati in `ANALYSIS_GIUFFRE_ENCICLOPEDIA.md` se emergeranno in fase di sviluppo, ma non sono attesi colpi di scena strutturali.
- **Manuali giuridici — sesto profilo UTET aggiunto (maggio 2026)**: il profilo `compendio_utet` (Tesauro, Compendio di Diritto Tributario) è il sesto del progetto. Caratterizzato dall'assenza totale di apparato critico (Layout 4 non applicabile). Pipeline editoriale identica al Mosconi (`manuale_utet_wolterskluwer`) ma genere editoriale opposto (compendio vs trattato). Per consolidamento del profilo servirebbe un secondo compendio UTET. Bassa priorità: il profilo è già coerente.
- **Sintesi trasversale dei sei manuali — `ANALYSIS_MANUALI_OVERVIEW.md` (maggio 2026)**: prodotto come documento canonico di sintesi che raccoglie le quattro architetture editoriali, le sette categorie semantiche introdotte, le quattro logiche di ricomposizione, le tre composizioni di heading di paragrafo, e le decisioni architetturali consolidate. Affianca i sei file di analisi singoli che restano canonici per i dettagli misurati con PyMuPDF.
- **`ARCHITECTURE.md` PRODOTTO (maggio 2026)**: copre Layer 1 + Layer 2 a livello di dettaglio medio, in inglese, come guida operativa con checklist organizzate per fase di sviluppo. Plugin-based dispatch per i profili. Schema JSON canonico in sezione dedicata. Meccanismo `layouts_disabled` con reason formalizzato. Da qui in poi le decisioni architetturali sono cristallizzate: la fase di analisi pre-sviluppo è conclusa.
- **Decisione architetturale `layouts_disabled` — formalizzata in `ARCHITECTURE.md` § 11.6**: confermata dalla doppia evidenza Patriarca-Benazzo + Tesauro + Torrente (zero note bibliografiche). Il document profile dichiara quali layout sono applicabili; il frontend disabilita e segnala accessibilmente quelli non applicabili.
- **Distinzione trasversale "trattato" vs "compendio"**: emersa da Mosconi vs Tesauro. Asse trasversale a tutti gli editori. Probabili future categorie: `compendio_giuffre`, `compendio_zanichelli`, `compendio_giappichelli` ecc. Decisione di nomenclatura non bloccante; eventuale rinomina futura di `manuale_utet_wolterskluwer` in `trattato_utet` per simmetria — non urgente.
- **Prossimo asse di lavoro**: la fase di analisi pre-sviluppo è conclusa. Il prossimo passaggio è il **coinvolgimento di Claude Code per l'inizio dell'implementazione del Layer 1** (pipeline Python). Le decisioni architetturali, il corpus, le categorie semantiche e lo schema JSON sono tutti documentati. Eventuali nuovi PDF caricati dall'utente (altri editori, manuali aggiuntivi) verranno gestiti additivamente dal meccanismo plugin-based, senza richiedere refactoring del core.
