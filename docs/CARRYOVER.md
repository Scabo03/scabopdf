...
> Versione: 1.1 (10 maggio 2026, dopo chiusura del Layer 1 § 4 — pipeline Python operativa fino al block classification tier 1 con dispatch al tier 2)
...
Il progetto è entrato in fase di sviluppo del Layer 1 (pipeline Python) il 9 maggio 2026 con lo scaffold del repository monorepo. Al 10 maggio 2026 sono completi e su `origin/main` i §§ 1-4 di Layer 1: project setup (`d82e8b6`), profiling foundations (`96cdc8e`), block extraction (`07b0d07` + `d9aa37b` + `9e1d360`), block classification tier 1 + dispatch al tier 2 (`040fdfd` + `e8ead6a`). Il Layer 2 (React Native in `app/`) e la CI/pre-commit non sono ancora inizializzati: rimandati a sessione dedicata in cui sarà disponibile MacInCloud per il primo build effettivo iOS.
...
**Codice di Layer 1**: `pipeline/` è uno scheletro funzionante con quattro moduli implementati (`extraction/`, `profiling/`, `classification/`, `schema/`) e sei stub vuoti per i moduli a valle (`apparatus/`, `emission/`, `postprocessing/`, `reconstruction/`, `utils/`, e `profiles/` con il solo `unknown_generic` aggiornato per implementare anche `refine_classification` come pass-through). 77 test unit verdi, coverage 97% complessiva. Il marker PEP 561 `py.typed` è in `src/scabopdf_pipeline/`, mypy strict è pulito sia su `src/` che su `tests/`.
...
### Sessione che ha prodotto questa versione del carryover (10 maggio 2026, nono aggiornamento — versione 1.1)
...
- **Layer 1 §§ 1-4 chiusi e su `origin/main`** al 10 maggio 2026. Quality gate del momento: 77 test verdi, coverage 97% complessiva, ruff e mypy strict puliti su `src/` e `tests/`. **Prossimo task naturale: § 5 structural reconstruction** (qui si riempie `reconstruction/types.py:Document`, vedi ARCHITECTURE.md § 5). Alternative: Blocco B (init React Native in `app/`) se si prevede una sessione MacInCloud nel breve, oppure sessione fixture reali (rinviata da molte sessioni, sempre più rilevante per coprire il branch Custom-encoding di § 3 e l'integration test round-trip char-count del checklist § 3).
- **Regole `CLAUDE.md` sono vincolanti**: niente menu interattivi, niente decisioni autonome su design/library/schema/architettura, chiarificazioni in prosa lineare con trade-off espliciti. Le sessioni di § 3 e § 4 hanno applicato il pattern (sette questioni in prosa per § 3, sei per § 4, risposte ricevute, poi implementazione end-to-end). È il modello per le sessioni successive.
- **Convenzioni di codice consolidate dai §§ 2, 3 e 4 (da rispettare a valle)**: dataclass `frozen` per i contenitori interni (`kw_only=True` quando l'ordine semantico dei campi richiede un default-prima-di-non-default — caso `ClassifiedBlock`), Pydantic `BaseModel` con `model_config = ConfigDict(frozen=True)` per ciò che esce in JSON; un solo modulo per ogni I/O esterno (`pymupdf_adapter.py` è l'unico import di `fitz`); `__init__.py` di modulo re-esporta solo i simboli pubblici; bool derivati da bitfield esposti come `@property` sopra il campo crudo; ranges half-open `(start, end)` per linkare contenitori a liste flat e indici flat per fare riferimento a un elemento (es. `ClassifiedBlock.block_index` punta a `ExtractionResult.blocks`, non al `Block.block_index` per-pagina); filtro precoce dei dati irrilevanti già in extraction quando ARCHITECTURE.md è esplicita su "Otherwise discarded"; vocabolari chiusi documentati come costanti modulo-livello (es. `TIER1_REASONS`) quando il dominio dei valori è enumerabile e citato in ARCHITECTURE.md; le modifiche di interfaccia astratta (es. nuovo metodo `@abstractmethod`) si bundlano con l'aggiornamento dei test che le rendono compilabili, per non lasciare lo stato intermedio rotto tra commit produzione e commit test.
